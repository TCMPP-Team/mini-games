...
module.exports = function(PIXI, app, obj, callBack) {
    let container = new PIXI.Container(),
        ...
        renameSyncButton = p_button(PIXI, {
            width: 580 * PIXI.ratio,
            y: renameButton.height + renameButton.y + 20 * PIXI.ratio
        });
    ...
    // 重命名 “按钮” 开始
    renameSyncButton.myAddChildFn(
        p_text(PIXI, {
            content: '同步重命名',
            fontSize: 36 * PIXI.ratio,
            fill: 0xffffff,
            relative_middle: { containerWidth: renameSyncButton.width, containerHeight: renameSyncButton.height }
        })
    );
    renameSyncButton.onClickFn(() => {
        callBack({
            status: 'renameSync',
            pathArr,
            drawFn() {
    ...
    container.addChild(goBack, title, api_name, underline, pathBox, tipText, renameButton, logo, logoName, renameSyncButton);
