...
module.exports = function(PIXI, app, obj, callBack) {
    ...
    // 点击保存 “按钮” 开始
    let buttonSync = p_button(PIXI, {
        width: 580 * PIXI.ratio,
        y: button.height + button.y + 20 * PIXI.ratio
    });
    buttonSync.myAddChildFn(
        p_text(PIXI, {
            content: '同步保存',
            fontSize: 36 * PIXI.ratio,
            fill: 0xffffff,
            relative_middle: { containerWidth: buttonSync.width, containerHeight: buttonSync.height }
        })
    );
    buttonSync.onClickFn(() => {
        callBack({
            status: 'saveFile',
            isSync: true,
            index: div.getChildIndex(pitch_on.parent)
    ...
    container.addChild(goBack, title, api_name, underline, div, tipText, button, logo, logoName, buttonSync);
