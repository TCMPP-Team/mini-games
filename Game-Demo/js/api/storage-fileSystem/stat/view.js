...
module.exports = function(PIXI, app, obj, callBack) {
    let container = new PIXI.Container(),
        ...
        statSyncButton = p_button(PIXI, {
            width: 580 * PIXI.ratio,
            y: statButton.height + statButton.y + 20 * PIXI.ratio
        }),
        pitch_on,
        div;
    ...
    // 点击判断 “按钮” 开始
    statSyncButton.myAddChildFn(
        p_text(PIXI, {
            content: '同步判断',
            fontSize: 36 * PIXI.ratio,
            fill: 0xffffff,
            relative_middle: { containerWidth: statSyncButton.width, containerHeight: statSyncButton.height }
        })
    );
    statSyncButton.onClickFn(() => {
        callBack({ status: 'statSync', index: div.getChildIndex(pitch_on.parent) });
    });
    // 点击判断 “按钮” 结束

    container.addChild(goBack, title, api_name, underline, div, statButton, statSyncButton, logo, logoName);
