...
module.exports = function(PIXI, app, obj, callBack) {
    ...
    progressBar.circle.onTouchMoveFn(e => {
        callBack({status: 'pause'});
        switchButtonFn('showFn', 'hideFn');

        if (e.data.global.x >= progressBar.gray.x && progressBar.gray.x + progressBar.gray.width >= e.data.global.x) {
            progressBar.circle.setPositionFn({ x: e.data.global.x });
            progressBar.green.width = e.data.global.x - progressBar.green.x;
            const time = progressBar.green.width / progressBar.gray.width;
            callBack({
                status: 'seek',
                value: (time * totalDuration).toFixed(3)
            });
            timingOperation((time * totalDuration).toFixed(3));
        }
    }); 
    ...
    playButton.onClickFn(() => {
        callBack({
            status: 'play', 
            cb: (status, duration, currentTime) => {
                switch (status) {
                    case 'play':
                        if (voiceBandEnded) {
                            resetProgressBar();
    ...
    function timingOperation(currentTime) {
        ...
        startingTime.turnText(`${minute / 60 < 10 ? '0' + minute : minute} : ${second % 60 < 10 ? '0' + second : second}`);
    }
    // 更新播放时间function 结束

    // 创建内部 audio 上下文 InnerAudioContext 对象 开始
    callBack({
        status: 'createInnerAudioContext', 
        cb: (status, duration, currentTime) => {
            switchButtonFn('hideFn', 'showFn');
            timingOperation(currentTime);
    ...
    goBack.callBack = callBack.bind(null, {
        status: 'destroy', 
        cb: () => {
            switchButtonFn('showFn', 'hideFn')
        }
