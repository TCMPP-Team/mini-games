...
module.exports = function(PIXI, app, obj, callBack) {
    let container = new PIXI.Container(),
        ...
        readdirSyncButton = p_button(PIXI, {
            width: 580 * PIXI.ratio,
            y: readdirButton.height + readdirButton.y + 20 * PIXI.ratio
        });
    ...
    // 同步点击查看 “按钮” 开始
    readdirSyncButton.myAddChildFn(
        p_text(PIXI, {
            content: '同步读取目录',
            fontSize: 36 * PIXI.ratio,
            fill: 0xffffff,
            relative_middle: { containerWidth: readdirSyncButton.width, containerHeight: readdirSyncButton.height }
        })
    );
    readdirSyncButton.onClickFn(() => {
        callBack({
            status: 'readdirSync',
            drawFn(path) {
                wx.getFileSystemManager().stat({
    ...
    container.addChild(goBack, title, api_name, underline, pathBox, readdirButton, logo, logoName, readdirSyncButton);
